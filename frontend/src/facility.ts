...
export const floors: FloorRecord[] = [
  ...
  { number: 7, name: "Engineering East", use: "Open workplace", occupancy: 46, capacity: 62, area: "18,400 sq ft", status: "Critical", openTickets: 1, rooms: ["East office zone", "Engineering west", "Electrical 7A", "Meeting 7C"] },
  ...
  { number: 4, name: "Client Services", use: "Meeting center", occupancy: 34, capacity: 56, area: "18,100 sq ft", status: "Warning", openTickets: 1, rooms: ["Conference 4B", "Client lounge", "Meeting 4A", "Support hub"] },
...
const baseSensors = (floor: FloorRecord): SensorRecord[] => {
  ...
  const temperature = floor.number === 4 ? "77.2°F" : `${(69.8 + (floor.number % 4) * 0.7).toFixed(1)}°F`;
  const temperatureState: SensorState = floor.number === 4 ? "Warning" : "Normal";
  return [
    ...
    { id: `PWR-${pad}-01`, floor: floor.number, area: floor.name, type: "Electrical load", value: `${78 + floor.number * 4} kW`, target: "< 145 kW", state: floor.number === 7 ? "Critical" : "Normal", seen: `${3 + floor.number}s ago` },
  ];
};

export const sensors: SensorRecord[] = floors.flatMap(baseSensors).map((sensor) => sensor.id === "PWR-07-01" ? { ...sensor, id: "ELEC-7A", area: "Floor 7 east panel", type: "Cabinet temperature", value: "126.4°F", target: "< 95°F" } : sensor);
