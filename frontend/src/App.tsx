...
function AgentLiveView({ live, busy, onPulse, onReview }: { live: LiveOperations; busy: boolean; onPulse: () => void; onReview: (ticket: Ticket) => void }) {
  ...
  return <section className="agent-live-view">
    <div className="engine-banner">
      <div><p>BACKGROUND OPERATIONS</p><h2>Parallel workflows. Specialist collaboration.</h2><span>{live.agent.worker_count} workers process tickets concurrently. Within each workflow, bounded specialists analyze evidence in parallel.</span></div>
    ...
    <div className="engines-grid">
      ...
      <section className="engine-card agent-engine">
        <div className="engine-card-head"><span className="engine-number">02</span><div><p>OPERATIONS SERVICE</p><h3>Coordinated specialist agents</h3><small>{live.agent.runtime} · {live.agent.provider} / {live.agent.model} · {live.agent.worker_count} workflow workers · Policy gateway</small></div><span className="engine-online"><i />{live.agent.real_model ? "AI MODEL LIVE" : "FIXTURE MODE"}</span></div>
        <div className="engine-flow"><span>Consume pub/sub</span><i>→</i><span>Specialists analyze</span><i>→</i><span>Coordinator + policy</span></div>
        <div className="specialist-roster"><div><small>COORDINATOR</small><b>{live.agent.coordinator_role}</b></div><i>←</i><section>{live.agent.specialist_roles.map((role) => <span key={role}><em>AI</em>{role.replace(" Intelligence", "").replace(" Agent", "")}</span>)}</section></div>
