...
| Live UI updates | Process-local SSE fanout | API Gateway WebSocket + EventBridge/SNS | event envelope |
...
2. Replace the job delivery adapter with SQS/Lambda while retaining SQLite for a controlled split test.
3. Move persistence to DynamoDB and run the same lifecycle/idempotency tests.
4. Replace read tools, then write tools one at a time; rerun policy tests after every swap.
5. Host the API/UI last. Change environment configuration, not service logic.
