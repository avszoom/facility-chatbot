...
function AgentProgress({ ticket, events }: { ticket: Ticket; events: TicketEvent[] }) {
  return <div className="agent-progress"><div className="agent-now"><span>AI</span><div><b>{activityCopy[ticket.status]}</b><small>Facility Operations Agent · {ticketStatusLabel(ticket.status)}</small></div></div><Pipeline ticket={ticket} eventTypes={events.map(e => e.event_type)} /></div>;
}
...
function MailPreview({ ticket, events }: { ticket: Ticket; events: TicketEvent[] }) {
  const message = [...events].reverse().find((event) => ["approval.requested", "message.sent", "staff.response_sent", "staff.note_added"].includes(event.event_type));
  const requiresAction = ["needs_approval", "escalated"].includes(ticket.status);
  const staffReply = ["staff.response_sent", "staff.note_added"].includes(message?.event_type || "");
...
function StaffResponseBox({ requester, busy, canResolve, onRespond }: { requester: string; busy: boolean; canResolve: boolean; onRespond: (response: string) => void }) {
  const [response, setResponse] = useState("");
  return <form className="staff-response" onSubmit={(event) => { event.preventDefault(); onRespond(response.trim()); }}><div><span>!</span><div><b>{canResolve ? "Answer the resident" : "Keep the resident informed"}</b><small>Send a simulated message to {requester}. {canResolve ? "Your answer will close this question." : "This update does not fix the workflow or close the request."}</small></div></div><textarea value={response} onChange={(event) => setResponse(event.target.value)} minLength={3} required placeholder={canResolve ? "Write the confirmed answer…" : "Explain what is being checked and what happens next…"} /><div className="staff-response-actions"><small>Saved in this request’s history. No external email is sent.</small><button disabled={busy || response.trim().length < 3}>{canResolve ? "Send answer & resolve" : "Send update · keep open"}</button></div></form>;
}
  ...
  return <>
    <section className="workdesk"><aside className="mailbox"><div className="mailbox-head"><div><h2>Requests</h2><span>{tickets.length} conversations</span></div><button className="mini-action" onClick={onCreate}>＋</button></div><div className="mailbox-tools"><button className={filter === "all" ? "selected" : ""} onClick={() => setFilter("all")}>All</button><button className={filter === "open" ? "selected" : ""} onClick={() => setFilter("open")}>Open <b>{metrics.active}</b></button><button className={filter === "attention" ? "selected" : ""} onClick={() => setFilter("attention")}>Needs attention <b>{attentionCount}</b></button><button className={filter === "resolved" ? "selected" : ""} onClick={() => setFilter("resolved")}>Resolved <b>{metrics.resolved}</b></button></div><div className="mail-list">{filteredTickets.length ? filteredTickets.map((ticket) => <button className={`mail-row ${selectedId === ticket.ticket_id ? "selected" : ""}`} onClick={() => onSelect(ticket.ticket_id)} key={ticket.ticket_id}><span className={`sender-avatar priority-${ticket.priority}`}>{initials(ticket.requester)}</span><span className="mail-copy"><span><b>{ticket.requester}</b><time>{ticketTime(ticket.updated_at)}</time></span><strong>{ticket.subject}</strong><small>{ticket.status === "needs_approval" ? "Your approval is needed" : ticket.status === "resolved" ? "Request completed" : ticket.status === "escalated" ? "Review why the agent stopped" : `Autopilot handling · ${ticket.waiting_reason || activityCopy[ticket.status]}`}</small><em className={`status ${ticket.status}`}>{ticketStatusLabel(ticket.status)}</em></span></button>) : <div className="empty-state"><b>No matching request mail</b><span>Choose another filter or create a new request.</span></div>}</div>{import.meta.env.DEV && <div className="mailbox-footer"><button onClick={onLoad} disabled={busy}>Load sample workspace</button><button onClick={onProcess} disabled={busy}>Process scheduled work</button></div>}</aside>
      <main className="request-workspace">{!detail ? <div className="empty-detail"><span>✉</span><h2>Select a request</h2><p>The resident message, agent work, and related building records will appear here.</p></div> : <>
        ...
        </section> : <AgentProgress ticket={detail.ticket} events={detail.events} />}
...
export default function App() {
  ...
  return <AppShell view={view} setView={setView} metrics={metrics}>
    ...
    {error && <p className="error-banner">Operations service unavailable. This workspace will reconnect automatically when service is restored.</p>}
    {view === "overview" && <OverviewView tickets={tickets} metrics={metrics} live={live} onReview={reviewTicket} />}
    {view === "requests" && <InboxView tickets={tickets} metrics={metrics} live={live} selectedId={selectedId} detail={detail} busy={busy} onSelect={setSelectedId} onCreate={() => setCreating(true)} onLoad={loadSamples} onProcess={() => operate(() => api.processScheduled())} onApprove={(approved) => detail && operate(() => api.approve(detail.ticket.ticket_id, approved))} onRespond={(response) => detail && operate(() => api.respond(detail.ticket.ticket_id, response, detail.review_summary?.can_resolve === true))} />}
